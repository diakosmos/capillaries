...
def flowsolve(QstpLm,D=1.0e-3,L=18e-2,T1=250.+273.15,P1=100.e3):
    global x
    global M
    global Tar
    global T0
...
#    P[0] = P1
    ...
    for i in range(1,nn):
        ...
        Tar[i] = Tstar * (g+1)/(2 + (g-1)*M[i]**2)
        Renar[i] = fRe(Q,Tar[i],D)
...
def fannoflow(D,L,T1,P1,nn=1000):
    global x, M, Tar, T0, P, P0, rho, a, u, fricint, turbar, Renar
    ...
    flowsolve(Qstp,D,L)
    T0 = Tar * (1 + ((g-1)/2)*M**2)   # should be constant!
    M1 = M[0]
    Tstar = T1 * (2 + (g-1)*M1**2)/(g+1)
    Pstar = P1 * M1 * sqrt((2 + (g-1)*M1**2)/(g+1))
    P0star = Pstar / .528
